...
cd ~/my-nextjs-app || exit

# Create components folder if not exists
mkdir -p components

# Wrapper file with dynamic import (no SSR)
cat > components/PurchaseAnalytics.tsx <<'EOF'
"use client";

import dynamic from "next/dynamic";

const PurchaseAnalyticsNoSSR = dynamic(
  () => import("./PurchaseAnalyticsInner"),
  { ssr: false }
);

export default PurchaseAnalyticsNoSSR;
EOF

# Inner file with full recharts code
cat > components/PurchaseAnalyticsInner.tsx <<'EOF'
...
export function PurchaseAnalytics({ user }: { user: User }) {
  ...
  const bundleData = Object.entries(bundleStats).map(([bundle, amount]) => ({ bundle, amount }));
  ...
  const pieData = bundleData.map(({ bundle, amount }) => ({ name: bundle, value: amount }));
...
echo "✅ PurchaseAnalytics files fixed with dynamic import (no SSR)."0
