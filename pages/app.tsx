...
export default function App() {
  ...
  // Example prices — replace with DB/API fetch if needed
  const agentPrices: Prices = {
    airtime: 9,
    bundles: [
      { name: "1GB", price: 8 },
      { name: "2GB", price: 15 },
  ...
  const currentPrices = role === "agent" ? agentPrices : customerPrices;
  ...
  const handleBundlePurchase = (bundle: Bundle) => {
    ...
    setTimeout(() => {
      alert(`Bundle purchased: ${bundle.name}`);
    ...
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex space-x-4">
        ...
        <button
          ...
          className={`px-4 py-2 rounded ${
            role === "agent" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          Agent
        </button>
      ...
      <div className="p-4 bg-gray-100 rounded shadow space-y-2">
        <h2 className="font-bold">
          Airtime ({role === "agent" ? "Agent Price" : "Customer Price"})
        </h2>
        <button
        ...
        >
          GHS {currentPrices.airtime}{" "}
          {airtimeLoading ? "(Processing...)" : ""}
        </button>
      </div>
      ...
      <div className="p-4 bg-gray-100 rounded shadow space-y-2">
        ...
        {currentPrices.bundles.map((bundle) => (
          ...
          >
            ...
            {bundleLoading ? "(Processing...)" : ""}
          </button>
        ))}
      </div>

      {/* Deposit Modal (toggle only, implement UI inside if needed) */}
      {showDepositModal && (
