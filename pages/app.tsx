  ...
  return (
    <div className="p-4 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="p-4 bg-gray-100 rounded shadow space-y-2">
        ...
        <input type="text" placeholder="Full Name" value={afaName} onChange={(e) => setAfaName(e.target.value)} className="w-full border p-2 rounded" />
        <input type="text" placeholder="Mobile Number" value={afaPhone} onChange={(e) => setAfaPhone(e.target.value)} className="w-full border p-2 rounded" />
        <input type="text" placeholder="Location" value={afaLocation} onChange={(e) => setAfaLocation(e.target.value)} className="w-full border p-2 rounded" />
        <input type="date" placeholder="Date of Birth" value={afaDob} onChange={(e) => setAfaDob(e.target.value)} className="w-full border p-2 rounded" />
        <button onClick={handleAfaRegister} className="w-full bg-purple-600 text-white p-2 rounded" disabled={afaLoading}>
          {afaLoading ? "Processing..." : "Register AFA"}
        </button>
      </div>
...
export default function App() {
  ...
  return (
    <div className="p-4 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-gray-100 p-4 rounded shadow flex justify-between items-center">
        <span>Balance: GHS {walletBalance.toFixed(2)}</span>
        {/* Green + button to trigger deposit */}
        <button
      ...
      <button
        ...
        disabled={afaLoading}
      >
        Register AFA ({role === "agent" ? "Agent" : "Customer"} price: GHS{" "}
        {currentPrices.afa})
      </button>
